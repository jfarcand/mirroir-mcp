...
/// Loads permission config from disk and resolves config directory paths.
///
/// Owns file I/O, CLI parsing, and directory resolution. The decision logic
/// lives in ``PermissionPolicy`` which consumes the loaded config.
public enum PermissionConfigLoader {
...
public struct PermissionPolicy: Sendable {
    ...
    // MARK: - Backwards-compatible static accessors

    /// Base directory name for config files (both global and project-local).
    public static var configDirName: String { PermissionConfigLoader.configDirName }
    /// Path to the global config directory.
    public static var globalConfigDir: String { PermissionConfigLoader.globalConfigDir }
    /// Path to the project-local config directory (relative to current working directory).
    public static var localConfigDir: String { PermissionConfigLoader.localConfigDir }
    /// Display path for error messages (shows the global config path with tilde).
    public static var configPath: String { PermissionConfigLoader.configPath }
    /// Load permission config from disk.
    public static func loadConfig() -> PermissionConfig? { PermissionConfigLoader.loadConfig() }
    /// Returns the skill directories in resolution order (project-local first, then global).
    public static var skillDirs: [String] { PermissionConfigLoader.skillDirs }
    /// Returns the agent profile directories in resolution order (project-local first, then global).
    public static var agentDirs: [String] { PermissionConfigLoader.agentDirs }
    /// Returns the prompt directories in resolution order (project-local first, then global).
    public static var promptDirs: [String] { PermissionConfigLoader.promptDirs }
    /// Parse CLI arguments for the skip-permissions flag.
    public static func parseSkipPermissions(from args: [String]) -> Bool {
        PermissionConfigLoader.parseSkipPermissions(from: args)
    }
    ...
    public func checkTool(_ toolName: String) -> PermissionDecision {
        ...
        if let denyList = config?.deny,
           denyList.contains(where: { $0.lowercased() == name }) {
            return .denied(reason:
                ...
                "Remove it from the deny list in \(PermissionConfigLoader.configPath) to allow it.")
        }
        ...
        if let allowList = config?.allow {
            ...
            return .denied(reason:
                ...
                "Add it to the allow list in \(PermissionConfigLoader.configPath) or use --dangerously-skip-permissions.")
        }

        return .denied(reason:
            "Tool '\(toolName)' requires explicit permission. " +
            "Use --dangerously-skip-permissions or configure \(PermissionConfigLoader.configPath).")
    }
        ...
        if let blockedApps = config?.blockedApps,
           blockedApps.contains(where: { $0.lowercased() == name }) {
            return .denied(reason:
                ...
                "Remove it from blockedApps in \(PermissionConfigLoader.configPath) to allow launching it.")
        }
