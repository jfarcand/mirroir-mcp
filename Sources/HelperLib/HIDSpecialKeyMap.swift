...
public enum HIDSpecialKeyMap {
    ...
    /// Uses `KeyName` as key type to ensure exhaustive coverage with `AppleScriptKeyMap`.
    ...
    /// Look up the USB HID keycode for a special key name string.
    ...
    /// Look up the USB HID keycode for a typed key name.
    public static func hidKeyCode(for key: KeyName) -> UInt16? {
        keyMap[key]
    }
